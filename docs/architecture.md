...
| Chart collars | Detected on the device (`ChartCoverage`) from the tiles themselves, cached in a `.coverage` sidecar, and clipped at render time — not cut in the tile pipeline | An FAA sheet is a whole printed page: its legend panel is over a degree of longitude wide and sits on top of the neighbouring chart. Cutting server-side is cleaner in principle but re-cuts nothing already downloaded — every user would have to re-fetch every chart — and it cannot help a sideloaded MBTiles at all |
| Chart layer stack | Downloaded MBTiles draw over a streaming layer, which skips any tile a download already covers in full | Having one state offline should not blank the rest of the country; suppressing covered tiles keeps the fill from re-fetching what is already on the device |
