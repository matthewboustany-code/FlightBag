...
| NOTAMs | `NotamStore.swift` (app) and `FAANotamProvider.swift` (FBProviders, NMS + OAuth token store). UI: `NotamSection.swift`, `NotamBriefingView.swift`, and the `.notam` branch of `MapAdvisories.swift`. The app never calls the FAA directly — everything goes through the server route |
